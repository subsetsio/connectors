...
def _child_entrypoint(fn: Callable, args: tuple, task_id: str, pipe_w) -> None:
    ...
    clear_tracking()
    raw_manifest.reset_pending()  # spawn starts fresh anyway; explicit for safety
...
class DAG:
    def __init__(
        ...
        for spec in specs:
            self.state[spec.id] = {
                ...
                "status": "pending",
                # Per-spec run OUTCOME, set only for successful nodes: one of
                # "skipped_fresh" (maintain said fresh), "ran_unchanged"
                # (download re-fetched identical bytes / transform publish
                # skipped), "ran_changed" (a real write happened), or "ran"
                # (executed but the change is unknowable). None for pending /
                # failed / blocked / filtered nodes. Additive observe-evidence
                # for the fleet freshness report — existing consumers ignore it.
                "outcome": None,
                "started_at": None,
                "finished_at": None,
                "duration_s": None,
    ...
    def _apply_maintain_skips(self, order: list[NodeSpec]) -> None:
        ...
        for spec in order:
            ...
            if fresh:
                ...
                self.state[spec.id]["outcome"] = "skipped_fresh"
    ...
    def _apply_result(self, task_id: str, result: dict) -> None:
        ...
        # Raw-manifest commit point (parent-only, single-threaded): a node that
        # completed successfully gets its staged raw-manifest entries merged
        # into the connector manifest here — objects were written first by the
        # child, the manifest entry commits strictly after, and dependents only
        # spawn once this node is `done`, so they resolve the fresh entries.
        # A failed node's staged entries are discarded (its objects stay
        # unreferenced — per the commit log they do not exist). Best-effort:
        # a commit hiccup falls back to run-scoped resolution downstream and
        # must not tank the node.
        raw_outcome: str | None = None
        if result["status"] == "done":
            # Classify the download change against the manifest's PREVIOUS
            # entries BEFORE commit_node consumes the staging and overwrites
            # them. Best-effort: a hiccup just leaves the outcome to default.
            try:
                raw_outcome = raw_manifest.classify_staged_change(task_id)
            except Exception as e:
                print(f"[DAG] WARN: raw-manifest classify failed for {task_id}: {type(e).__name__}: {e}")
            try:
                raw_manifest.commit_node(task_id)
            except Exception as e:
                print(f"[DAG] WARN: raw-manifest commit failed for {task_id}: {type(e).__name__}: {e}")
        else:
        ...
        # Classify the per-spec run OUTCOME (observe-evidence for the fleet
        # freshness report). Only successful, actually-executed nodes get one;
        # maintain-skips already stamped "skipped_fresh" up-front. Runs after
        # the merge so the transform publish-skip flag (surfaced through the
        # tracking snapshot) is visible in this process.
    ...
    def _resolve_outcome(self, task_id: str, raw_outcome: str | None) -> str:
        ...
        Transform-style nodes materialize subsets: the publish-skip flag rides
        the tracking snapshot (record_write unchanged=...) — every
        materialization skipped → "ran_unchanged"; any real write → "ran_changed".
        Download-style nodes use the raw-manifest staged-vs-committed compare
        (`raw_outcome`). Anything else executed, or an unknowable change, is
        plain "ran"."""
        mats = [w for w in get_assets_by_writer(task_id) if w.startswith("subsets/")]
        if mats:
            flags = [get_asset_version(w) or {} for w in mats]
            if all(f.get("unchanged") for f in flags):
                return "ran_unchanged"
            return "ran_changed"
...
def load_nodes(nodes_dir: Path | str | None = None) -> DAG:
    ...
    File-based transforms are loaded from the sibling `transforms/` directory
    (`<table>.sql` + `<table>.yml` pairs — see transforms.py). A file-based
    spec whose id collides with a module-authored spec REPLACES it, loudly:
    this is the catalog migration path — dropping an explicit file into
    transforms/ overrides the generic comprehension entry for that table.
    """
    ...
    file_specs = load_transform_dir(nodes_dir.parent / "transforms")

    if not nodes_dir.exists() and not file_specs:
    ...
    if file_specs:
        file_ids = {s.id for s in file_specs}
        overridden = [s.id for s in all_specs if s.id in file_ids]
        if overridden:
            print(f"[transforms/] overriding {len(overridden)} module-authored spec(s): "
                  f"{overridden[:5]}{'...' if len(overridden) > 5 else ''}")
