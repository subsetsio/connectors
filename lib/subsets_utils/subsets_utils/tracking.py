...
def current_task() -> str | None:
    """The currently executing task ID, or None outside a DAG node. Used by
    raw_manifest to route staged entries to the node's staging file."""
...
def record_write(asset_path: str, *, version: int = None, hash: str = None,
    ...
    `unchanged=True` marks a skip-unchanged publish (delta.WriteResult.unchanged):
    a materialization recorded at its EXISTING version because the source was
    byte-identical to the last commit. It rides the snapshot/merge pipe in
    `_asset_versions` alongside version/hash so the supervisor can classify the
    node's run outcome (ran_unchanged vs ran_changed) from the same channel.
    """
    ...
    with _lock:
        ...
        if version is not None:
            _asset_versions[asset_path] = {
                "version": version, "hash": hash, "unchanged": bool(unchanged),
            }
