"""OpenAlex subfields — third level of the topic hierarchy from the S3 snapshot."""
...
def _flat(r):
    ...
    return {
        ...
        "topics": join(short(t.get("id")) for t in (r.get("topics") or [])),
...
def fetch(node_id: str) -> None:
    save_raw_ndjson(iter_entity_rows("subfields", _flat), node_id)


_DOWNLOAD_SPECS = [
    NodeSpec(id="openalex-subfields", fn=fetch, kind="download"),
]

_TRANSFORM_SPECS = [
    SqlNodeSpec(
        id="openalex-subfields-transform",
        deps=["openalex-subfields"],
        sql='''
