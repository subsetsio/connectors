SELECT
    ...
    CASE WHEN TRY_CAST(expected AS DOUBLE) <= -9999 THEN NULL ELSE CAST(expected AS DOUBLE) END AS expected,
    ...
    CASE WHEN TRY_CAST(std_error AS DOUBLE) <= -9999 THEN NULL ELSE CAST(std_error AS DOUBLE) END AS std_error,
...
FROM "dartmouth-atlas-of-health-care-post-discharge-events"
