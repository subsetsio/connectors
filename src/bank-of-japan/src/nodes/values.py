"""Bank of Japan — values (observations) subset.

``values`` is the observations: long-format (db, series_code, frequency,
survey_date, value, last_update), sourced from /getDataCode.

Fetch shape: a content-signature incremental crawl. The full corpus is too
large to re-pull every run (CO/TANKAN alone is ~168k series), and the API
exposes no server-side ``since`` filter — only a per-series LAST_UPDATE that
can be diffed client-side. So we bucket series by (db, frequency), and for each
bucket store a signature ``max(last_update):n_series``. A bucket whose signature
is unchanged is skipped; a changed bucket is re-pulled. The crawl sweeps every
bucket; bucket state is checkpointed after each chunk, so if the supervisor
interrupts the node the next run resumes from the unfinished buckets — buckets
already current are skipped. Duplicate rows produced by re-pulling a revised
bucket are collapsed in the transform by keeping the highest last_update per
(db, series_code, survey_date). getDataCode requires all codes in one request
to share a frequency (hence bucketing by frequency); it caps at 250 codes and
60000 data points per request, paging the data-point overflow via
