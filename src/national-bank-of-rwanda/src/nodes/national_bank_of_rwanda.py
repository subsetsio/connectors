        ...
        sql='''
            WITH parsed AS (
                SELECT
                    ...
                    strptime(post_date, '%d-%b-%y')::DATE                   AS date,
                    CAST(REPLACE(buying_rate,  ',', '') AS DOUBLE)          AS buying_rate,
                    CAST(REPLACE(average_rate, ',', '') AS DOUBLE)         AS average_rate,
                    CAST(REPLACE(selling_rate, ',', '') AS DOUBLE)        AS selling_rate
                FROM "national-bank-of-rwanda-exchange-rates"
                WHERE post_date IS NOT NULL AND average_rate IS NOT NULL
            )
