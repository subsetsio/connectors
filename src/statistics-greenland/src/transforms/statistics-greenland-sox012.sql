...
-- caution: Dimension column(s) `no of children`, `no of adults`, `district` carry source-provided aggregate members (labels such as 'Total' / 'All Greenland') that summarise the other members of the same dimension alongside them. Filter each of those columns to a single level before summing `obs_value`, or the aggregate rows are counted twice.
...
SELECT
    ...
    "obs_value"
FROM "statistics-greenland-sox012"
