...
-- caution: `value` is untyped across rows: what it measures is selected by `datatype_code`, `estimate_code` and `provision_code`. Never aggregate `value` without first pinning those dimensions to a single measure.
