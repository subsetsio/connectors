...
-- caution: Upstream labelling bug: blocks 0 and 1 both carry the caption 'Average Value Weighted Returns -- Weekly' but hold different data (block 1 is the equal-weighted series, mislabelled by the source). `statistic` is therefore NOT unique per sub-table here — use `block` to separate them.
SELECT
    "block",
    "statistic",
    "period",
    ...
    "variable",
    "value"
FROM "kenneth-french-data-library-6-portfolios-2x3-weekly"
