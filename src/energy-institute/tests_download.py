...
# spec_ids is every DAG node id (downloads AND the transform leaf). The
# transform node publishes a Delta table, not a raw parquet, so restrict the
# raw-asset invariants to the download fetch(es) only.
