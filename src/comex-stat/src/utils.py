"""Shared helpers for the Comex Stat connector.

The bulk file host (balanca.economia.gov.br, *.economia.gov.br via SERPRO) serves
a TLS leaf certificate WITHOUT its issuing intermediate. Browsers and system curl
recover the missing "Sectigo Public Server Authentication CA OV R36" intermediate
via AIA fetching, but certifi/OpenSSL do not, so a default httpx verify fails with
"unable to get local issuer certificate". Rather than disable verification, we ship
the intermediate here and build a combined CA bundle (certifi roots + intermediate)
that completes the chain — verification stays ON.
"""

import os
import tempfile

import certifi

# Sectigo Public Server Authentication CA OV R36 — the intermediate the origin
# omits from its TLS handshake. Issued by Sectigo Public Server Authentication
# Root R46 (present in certifi). Fetched from http://crt.sectigo.com/.
_SECTIGO_OV_R36_INTERMEDIATE = """-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
"""

# A distinctive line from the intermediate's base64 body, used to detect whether
# the bundle already carries it (idempotency marker).
_MARKER = "MIIGTDCCBDSgAwIBAgIQLBo8dulD3d3"


def ensure_ca() -> None:
    """Append the omitted Sectigo intermediate to certifi's CA bundle in place, so
    the process-global httpx client (which loads certifi at creation) can build a
    complete chain to the origin. Idempotent; call before the first HTTP request in
    a fetch fn. Verification stays enabled — we are completing the chain, not
    bypassing it."""
    path = certifi.where()
    with open(path, "r", encoding="ascii") as f:
        current = f.read()
    if _MARKER in current:
        return
    with open(path, "a", encoding="ascii") as f:
        f.write("\n" + _SECTIGO_OV_R36_INTERMEDIATE)
