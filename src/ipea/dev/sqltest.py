...
s=con.execute('''SELECT SERCODIGO AS series_code, SERNOME AS name, BASNOME AS database, TEMCODIGO AS theme_code, CAST(SERATUALIZACAO AS TIMESTAMP) AS last_updated, SERNUMERICA AS is_numeric FROM "ipea-series" WHERE SERCODIGO IS NOT NULL''').arrow()
print("series transform rows:", s.num_rows, "cols:", s.column_names)
print(s.slice(0,2).to_pydict())
v=con.execute('''SELECT SERCODIGO AS series_code, CAST(substr(VALDATA,1,10) AS DATE) AS date, CAST(VALVALOR AS DOUBLE) AS value, NULLIF(NIVNOME,'') AS geo_level, NULLIF(TERCODIGO,'') AS territory_code FROM "ipea-values" WHERE VALVALOR IS NOT NULL AND VALDATA IS NOT NULL''').arrow()
