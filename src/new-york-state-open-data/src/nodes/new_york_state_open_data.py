...
One download spec per accept-accepted dataset (the entity union in
...
Python list — because several accepted tables run to tens of millions of rows
and would OOM a buffering fetch. Raw is written as gzip-compressed CSV
(`<asset>.csv.gz`); DuckDB's read_csv_auto infers the gzip from the suffix, so
the downstream profiler/transform read it transparently.

Shape: stateless full re-pull (shape 1). Socrata supports a SoQL `$where`
delta filter, but our pattern is full snapshots, so we re-fetch each dataset
whole every run and overwrite. The bulk export's header row carries the
dataset's display column names.
...
SKIP_TTL = 14 * 86400
# Generous read timeout: a large table's export can take a while to start
# streaming, and each chunk must arrive within this window.
...
def _stream_csv(asset: str, url: str) -> None:
    """Stream the bulk CSV export to the raw store as gzip. Re-entrant: each
    attempt truncates and rewrites, so a mid-stream transient failure retries
    cleanly (the whole export is re-requested)."""
    client = get_client()
    with client.stream(
