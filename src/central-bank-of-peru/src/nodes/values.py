...
Sourced from the per-series JSON API ``/api/{codes}/json/{start}/{end}/ing``.
Up to 10 codes of the SAME frequency are batched per request (the API caps at
10 and rejects mixed frequencies). The full ~17,100-series corpus is ~1,700
...
from utils import (
    _C_CODE, _C_FREQ, _C_START, _C_END,
    _cell,
    _fetch_catalog_rows,
    _http_get,
)
