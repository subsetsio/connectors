...
now=int(time.time())
for days in [15, 20, 30, 45]:
    r=get("https://clob.polymarket.com/prices-history",
          params={"market":tok,"startTs":now-days*86400,"endTs":now,"fidelity":60},timeout=(10,90))
    h=r.json().get("history",[]) if r.status_code==200 else []
    print(f"{days}d window fid60: status {r.status_code} pts {len(h)}")
