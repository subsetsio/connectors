-- compiled by `hardened compile-transforms` from the measured model
-- profiles (model/tables + columns). Faithful pass-through: verified
-- pure casts only, no data fixes. Regenerate after model-verify;
-- durable edits belong in the model stage, not here.
-- caution: Each `section` is an independent cross-tab of the same population (AGE, BROAD AGE GROUPS, MARITAL STATUS, SEX); 'AGE' and 'BROAD AGE GROUPS' are two partitions of the same ages at different resolutions. Sum within one section, never across.
-- caution: Both `category` and `breakdown` carry a 'Total' member alongside their components — filter both out before aggregating.
-- caution: Periods are U.S. federal fiscal years (October 1 to September 30), not calendar years.
-- caution: Counts are rounded to the nearest 10 by OHSS, so component rows need not sum exactly to the published totals.
SELECT
    "topic",
    "table_label",
    "title",
    "section",
    "parent_category",
    "category",
    "description",
    "breakdown",
    "value",
    "value_note"
FROM "dhs-refugees-table-15"
