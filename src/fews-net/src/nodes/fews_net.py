...
TRANSFORM_SPECS = [
    SqlNodeSpec(
        id="fews-net-ipcphase-transform",
        deps=["fews-net-ipcphase"],
        sql='''
            SELECT
                ...
                CAST(projection_start AS DATE)    AS projection_start,
                CAST(projection_end AS DATE)      AS projection_end,
                CAST(value AS DOUBLE)             AS phase,
    ...
    SqlNodeSpec(
        id="fews-net-ipcpopulationsize-transform",
        deps=["fews-net-ipcpopulationsize"],
        sql='''
            SELECT
                CAST(id AS BIGINT)                AS id,
                country,
                ...
                CAST(projection_start AS DATE)    AS projection_start,
                CAST(projection_end AS DATE)      AS projection_end,
                CAST(low_value AS DOUBLE)         AS population_low,
                CAST(high_value AS DOUBLE)        AS population_high,
                population_range,
                indicator_name,
                collection_schedule,
                collection_status,
    ...
    SqlNodeSpec(
        id="fews-net-marketpricefacts-transform",
        deps=["fews-net-marketpricefacts"],
        sql='''
            SELECT
                CAST(id AS BIGINT)                AS id,
                country,
                ...
                common_unit,
                common_currency,
                CAST(common_unit_price AS DOUBLE)     AS common_unit_price,
                CAST(common_currency_price AS DOUBLE) AS common_currency_price
            FROM "fews-net-marketpricefacts"
            WHERE value IS NOT NULL
        ''',
    ),
    SqlNodeSpec(
        id="fews-net-tradeflowquantityvalue-transform",
        deps=["fews-net-tradeflowquantityvalue"],
        sql='''
            SELECT
                CAST(id AS BIGINT)                AS id,
                reporting_country,
                ...
                unit_name,
                common_unit,
                CAST(common_unit_quantity AS DOUBLE) AS common_unit_quantity,
                source_organization,
                source_document,
                collection_status
