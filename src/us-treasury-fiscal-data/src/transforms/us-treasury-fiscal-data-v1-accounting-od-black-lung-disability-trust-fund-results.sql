...
-- caution: Aggregate rows sit alongside the components they sum: `component_desc` ('Total expenses', 'Total outlays'); `line_item_desc` ('Total expenses', 'Total outlays', 'Total receipts'). Summing an amount column without excluding them double-counts.
SELECT
    "record_date",
    ...
    "component_desc",
    "line_item_desc",
    CAST(NULLIF("fiscal_year_amt", 'null') AS DOUBLE) AS fiscal_year_amt,
    ...
    "record_calendar_day"
FROM "us-treasury-fiscal-data-v1-accounting-od-black-lung-disability-trust-fund-results"
