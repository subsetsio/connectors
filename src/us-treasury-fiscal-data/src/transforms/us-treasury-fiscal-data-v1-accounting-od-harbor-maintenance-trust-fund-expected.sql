...
-- caution: Aggregate rows sit alongside the components they sum: `component_desc` ('Total available', 'Total outlays', 'Total receipts'). Summing an amount column without excluding them double-counts.
...
SELECT
    ...
    "account_desc",
    "component_desc",
    CAST(NULLIF("fiscal_year_expected_mil_amt", 'null') AS DOUBLE) AS fiscal_year_expected_mil_amt,
    ...
    "record_calendar_day"
FROM "us-treasury-fiscal-data-v1-accounting-od-harbor-maintenance-trust-fund-expected"
