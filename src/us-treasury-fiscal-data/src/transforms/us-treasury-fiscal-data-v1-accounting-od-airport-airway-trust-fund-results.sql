...
-- caution: Aggregate rows sit alongside the components they sum: `component_desc` ('Total expenses', 'Total receipts'); `line_item_desc` ('Total expenses', 'Total receipts', 'Total refunds of taxes'). Summing an amount column without excluding them double-counts.
SELECT
    "record_date",
    ...
    "line_item_desc",
    "irc_section_cd",
    CAST(NULLIF("fiscal_year_amt", 'null') AS BIGINT) AS fiscal_year_amt,
    ...
    "record_calendar_day"
FROM "us-treasury-fiscal-data-v1-accounting-od-airport-airway-trust-fund-results"
