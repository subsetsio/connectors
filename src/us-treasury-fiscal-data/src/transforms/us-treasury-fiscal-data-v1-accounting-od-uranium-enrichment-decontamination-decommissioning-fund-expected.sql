...
-- caution: Aggregate rows sit alongside the components they sum: `component_desc` ('Total Outlays net of Investments Redeemed', 'Total Receipts'). Summing an amount column without excluding them double-counts.
...
SELECT
    ...
    "account_desc",
    "component_desc",
    CAST(NULLIF("fiscal_year_expected_thous_amt", 'null') AS BIGINT) AS fiscal_year_expected_thous_amt,
    ...
    "record_calendar_day"
FROM "us-treasury-fiscal-data-v1-accounting-od-uranium-enrichment-decontamination-decommissioning-fund-expected"
