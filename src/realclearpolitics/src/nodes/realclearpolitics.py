    ...
    SqlNodeSpec(
        ...
        sql='''
            SELECT
                ...
                pollster,
                date_label,
                TRY_CAST(strptime(data_start_date, '%Y/%m/%d') AS DATE) AS poll_start_date,
                TRY_CAST(strptime(data_end_date,   '%Y/%m/%d') AS DATE) AS poll_end_date,
                TRY_CAST(regexp_extract(sample_size_raw, '[0-9]+') AS INTEGER) AS sample_size,
                NULLIF(regexp_extract(sample_size_raw, '[A-Za-z]+$'), '')      AS sample_population,
                TRY_CAST(margin_error AS DOUBLE)                AS margin_error,
                NULLIF(partisan, '')                            AS partisan,
                candidate,
                affiliation,
