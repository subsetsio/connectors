...
# --------------------------------------------------------------------------- #
# Transforms — one published Delta table per subset (thin parse-and-type)     #
# --------------------------------------------------------------------------- #
def _sql_flask_std(dep):
    # Scripps quality flags: <= 0 accepted (0 = accepted flask, -1/-2/-3 =
    # accepted continuous-analyzer / retained), >= 1 rejected (analysis errors,
    # contaminated flasks). Rejected rows carry absurd values (e.g. 8700 ppm),
    # so publish accepted samples only; the flag is kept for provenance.
    return f'''
        SELECT station,
...
def _sql_flask_isotopes(dep):
    # Each species carries its own quality flag; null any species value whose
    # flag rejects it (>= 1), keeping only accepted (<= 0) measurements.
    return f'''
        SELECT station,
               CAST(date AS DATE)           AS date,
               CAST(decimal_date AS DOUBLE) AS decimal_date,
...
def _sql_insitu_monthly(dep):
    return f'''
        SELECT station,
               CAST(year AS INTEGER)  AS year,
               CAST(month AS INTEGER) AS month,
               make_date(CAST(year AS INTEGER), CAST(month AS INTEGER), 15) AS date,
               CAST(sub_station AS VARCHAR)                     AS sub_station,
               CAST(decimal_date AS DOUBLE)                     AS decimal_date,
               CAST(value AS DOUBLE)                            AS value,
               CAST(value_seasonally_adjusted AS DOUBLE)        AS value_seasonally_adjusted,
               CAST(fit AS DOUBLE)                              AS fit,
...
_SQL = {
    ...
    "flask_monthly": _sql_flask_monthly,
    "insitu_monthly": _sql_insitu_monthly,
    "insitu_daily": _sql_insitu_daily,
    "insitu_weekly": _sql_insitu_weekly,
    "intermittent_c14": _sql_intermittent_c14,
    "intermittent_c14_spline": _sql_decimal_value,
    "mlo_spo_annual": _sql_mlo_spo_annual,
...
TRANSFORM_SPECS = [
    SqlNodeSpec(
        ...
        deps=[spec.id],
        sql=_SQL[_family(spec.id)](spec.id),
    )
